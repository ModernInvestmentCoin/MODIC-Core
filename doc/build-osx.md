...
====================================
This guide will show you how to build modicd (headless client) for OSX.
...
1. Clone the github tree to get the source code and go into the directory.
        ...
        git clone https://github.com/ModernInvestmentCoin/ModernInvestmentCoin.git
        cd ModernInvestmentCoin
...
5.  (Optional) You can also install modicd to your path:
...
4. Enter "modic-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `modicd` for your own use.

modicd/modic-cli binaries are not included in the modic-Qt.app bundle.

If you are building `modicd` or `modic-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the ModernInvestmentCoin-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./modicd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./modicd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=modicrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/ModernInvestmentCoin/modic.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/ModernInvestmentCoin/modic.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/ModernInvestmentCoin/debug.log

Other commands:
-------

    ./modicd -daemon # to start the modic daemon.
    ./modic-cli --help  # for a list of command-line options.
    ./modic-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure ModernInvestmentCoin with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the ModernInvestmentCoin wallet:
...
You have the choice to build the GUI ModernInvestmentCoin wallet as a Mac OSX app, described in “How to build the ModernInvestmentCoin-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the ModernInvestmentCoin-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find ModernInvestmentCoin-Qt.dmg inside your ModernInvestmentCoin folder. Open and install the wallet like any typical Mac app.
...
~/Downloads/ModernInvestmentCoin/src/qt/modic-qt