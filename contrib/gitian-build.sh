...
VERSION=
commit=false
url=https://github.com/ModernInvestmentCoin/MODIC-Core
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the modic, gitian-builder, gitian.sigs, and modic-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/ModernInvestmentCoin/MODIC-Core
...
then
	# Make output folder
	mkdir -p "./modic-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit modic=${COMMIT} --url modic=${url} ../modic/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../modic/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/modic-*.tar.gz build/out/src/modic-*.tar.gz ../modic-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit modic=${COMMIT} --url modic=${url} ../modic/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/modic-*.zip build/out/modic-*.exe ../modic-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit modic=${COMMIT} --url modic=${url} ../modic/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/modic-*.tar.gz build/out/modic-*.dmg ../modic-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit modic=${COMMIT} --url modic=${url} ../modic/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../modic/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/modic-*.tar.gz build/out/src/modic-*.tar.gz ../modic-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../modic/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/modic-*win64-setup.exe ../modic-binaries/${VERSION}
	    mv build/out/modic-*win32-setup.exe ../modic-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../modic/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/modic-osx-signed.dmg ../modic-binaries/${VERSION}/modic-${VERSION}-osx.dmg
	fi
	popd || exit
