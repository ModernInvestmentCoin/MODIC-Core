...
// Copyright (c) 2017-2020 ModernInvestmentCoin developers and researchers
...
#if defined(NDEBUG)
#error "ModernInvestmentCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umodic) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zMODIC is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zMODIC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zMODIC without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zMODIC spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zMODIC spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMODIC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmodic");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zMODIC mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetBlockValue(int nHeight)
{
    ...
    switch (nHeight){
        case       1  ...      70: nSubsidy = 20000 * COIN; break;
        case      71  ...   20160: nSubsidy = 0.00004567 * nHeight * COIN; break;
        case   20161  ...   40320: nSubsidy = 0.000114   * nHeight * COIN; break;
        case   40321  ...   60480: nSubsidy = 0.000137   * nHeight * COIN; break;
        case   60481  ...   80640: nSubsidy = 0.000147   * nHeight * COIN; break;
        case   80641  ...  100800: nSubsidy = 0.000152   * nHeight * COIN; break;
        case   100801 ...  120960: nSubsidy = 0.000155   * nHeight * COIN; break;
        case   120961 ...  141120: nSubsidy = 0.000158   * nHeight * COIN; break;
        case   141121 ...  161280: nSubsidy = 0.0001597  * nHeight * COIN; break;
        case   161281 ...  181440: nSubsidy = 0.000161   * nHeight * COIN; break;
        case   181441 ...  201600: nSubsidy = 0.000162   * nHeight * COIN; break;
        case   201601 ...  221760: nSubsidy = 0.000163   * nHeight * COIN; break;
        case   221761 ...  241920: nSubsidy = 0.0001637  * nHeight * COIN; break;
        case   241921 ...  262080: nSubsidy = 0.00016435 * nHeight * COIN; break;
        case   262081 ...  282240: nSubsidy = 0.0001648  * nHeight * COIN; break;
        case   282241 ...  302400: nSubsidy = 0.0001652  * nHeight * COIN; break;
        case   302401 ...  322560: nSubsidy = 0.0001656  * nHeight * COIN; break;
        case   322561 ...  342720: nSubsidy = 0.0001659  * nHeight * COIN; break;
        case   342721 ...  362880: nSubsidy = 0.0001662  * nHeight * COIN; break;
        case   362881 ...  383040: nSubsidy = 0.0001664  * nHeight * COIN; break;
        case   383041 ...  403200: nSubsidy = 0.0001667  * nHeight * COIN; break;
        case   403201 ...  500000: nSubsidy = 0.0001528  * nHeight * COIN; break;
        case   500001 ... 1000000: nSubsidy = (76.4 - 0.000081 * (nHeight - 500000)) * COIN; break;
        case  1000001 ... 1593993: nSubsidy = 35.9 * COIN; break;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ModernInvestmentCoin
...
{
    RenameThread("modic-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMODICMinted()
{
...
void RecalculateZMODICSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMODIC supply
...
bool RecalculateMODICSupply(int nHeightStart)
{
...
bool UpdateZMODICSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    UpdateZMODICSupply(block, pindex);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMODICSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MODIC to zMODIC
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zMODIC spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zMODIC serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMODIC mints and %d zMODIC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/ModernInvestmentCoin Core:1.1.0/" ||
            pfrom->cleanSubVer == "/ModernInvestmentCoin Core:1.3.0/" ||
            pfrom->cleanSubVer == "/ModernInvestmentCoin Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // ModernInvestmentCoin: We use certain sporks during IBD, so check to see if they are
