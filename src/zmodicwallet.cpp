...
CzMODICWallet::CzMODICWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmodic seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMODICSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMODICSeed_deprecated()) {
                    LogPrintf("%s: Updated zMODIC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmodic seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmodic wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMODICWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMODICCount(nCountLastUsed);
    else if (!walletdb.ReadZMODICCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMODICWallet::Lock()
{
    seedMaster = 0;
}

void CzMODICWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMODICWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMODIC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMODICWallet::LoadMintPoolFromDB()
{
...
void CzMODICWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMODICWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMODICWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmodicTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMODICWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMODIC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmodicTracker which also adds to database
    pwalletMain->zmodicTracker->Add(dMint, true);
...
void CzMODICWallet::SeedToZMODIC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMODICWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMODICWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZMODICCount(nCountLastUsed);
...
void CzMODICWallet::GenerateDeterministicZMODIC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMODICWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMODICWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
