...
bool CMasternodeConfig::read(std::string& strErr)
{
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:8822 891Bpm5f75D9qxqfUg8dfigi8NT4PtXzgBCEV4ZMcdC1qefw3re a7d152f3f02785cd529dcf8983bcdbd66551f7e6a53adab432a65ca7819ff644 0\n";
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (CService(ip).GetPort() != 8822) {
                strErr = _("Invalid port detected in masternode.conf") + "\n" +
                         strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                         _("(must be 8822 for mainnet)");
                streamConfig.close();
                return false;
            }
        } else if (CService(ip).GetPort() == 8822) {
            strErr = _("Invalid port detected in masternode.conf") + "\n" +
                     strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                     _("(8822 could be used only on mainnet)");
            streamConfig.close();
            return false;
        }
