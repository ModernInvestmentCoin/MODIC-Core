...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MODIC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMODIC send
enum ZerocoinSpendStatus {
    ZMODIC_SPEND_OKAY = 0,                            // No error
    ZMODIC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMODIC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMODIC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMODIC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMODIC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMODIC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMODIC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMODIC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMODIC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMODIC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMODIC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMODIC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMODIC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMODIC_SPENT_USED_ZMODIC = 14,                      // Coin has already been spend
    ZMODIC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZMODIC_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZModicBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMODICOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmodicAuto) const;
    ...
    */
    ...
    CzMODICWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMODICTracker> zmodicTracker;
    ...
    void setZWallet(CzMODICWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmodicTracker = std::unique_ptr<CzMODICTracker>(new CzMODICTracker(strWalletFile));
    }

    CzMODICWallet* getZWallet() { return zwalletMain; }
    ...
    void setZModicAutoBackups(bool fEnabled)
    {
