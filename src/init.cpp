...
CzMODICWallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "modic.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "modicd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MODIC and zMODIC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8822, 9822));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MODIC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MODIC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, modic, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MODIC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ModernInvestmentCoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzmodic=<n>", strprintf(_("Enable automatic wallet backups triggered after each zModic minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmodicbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMODIC backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizemodicamount=<n>     " + strprintf(_("Keep N MODIC anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8821, 9821));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i ModernInvestmentCoin developers and researchers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that ModernInvestmentCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ModernInvestmentCoin Core is shutting down."));
...
#endif
    // Make sure only a single ModernInvestmentCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ModernInvestmentCoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ModernInvestmentCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //ModernInvestmentCoin specific: zerocoin and spork DB's
                ...
                // ModernInvestmentCoin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZMODICMinted();
                        RecalculateZMODICSpent();
                    }
                    RecalculateMODICSupply(1);
                }
                ...
                // ModernInvestmentCoin: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ModernInvestmentCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ModernInvestmentCoin Core to complete") << "\n";
        ...
        zwalletMain = new CzMODICWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zMODIC wallet..."));

        bool fEnableZModicBackups = GetBoolArg("-backupzmodic", true);
        pwalletMain->setZModicAutoBackups(fEnableZModicBackups);
        ...
        pwalletMain->zmodicTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeModernInvestmentCoinAmount = 2;
...
//    }
//
//    nAnonymizeModernInvestmentCoinAmount = GetArg("-anonymizemodicamount", 0);
//    if (nAnonymizeModernInvestmentCoinAmount > 999999) nAnonymizeModernInvestmentCoinAmount = 999999;
//    if (nAnonymizeModernInvestmentCoinAmount < 2) nAnonymizeModernInvestmentCoinAmount = 2;
    ...
    LogPrintf("Anonymize ModernInvestmentCoin Amount %d\n", nAnonymizeModernInvestmentCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1MODIC+1000 == (.1MODIC+100)*10
       10MODIC+10000 == (1MODIC+1000)*10
    */
