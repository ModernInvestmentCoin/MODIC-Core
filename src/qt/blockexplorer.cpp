...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::MODIC, nValue);
...
{
    std::string genesisblockhash = "000004cd0b7e3f050671ae041632a1a7c9578b3ef55da7d08da9695f7ea44e0a";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (modic.conf).");
            QMessageBox::warning(this, "ModernInvestmentCoin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
