                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zMODICpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zMODIC ought to be enough for anybody." - Bill Gates, 2017
    ui->zMODICpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMODICSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMODICpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMODIC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMODIC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMODIC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMODIC in ") +
...
void PrivacyDialog::on_pushButtonSpendzMODIC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMODIC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMODIC
        sendzMODIC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMODIC
    sendzMODIC();
}

void PrivacyDialog::on_pushButtonZModicControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZModicControlDialog* zModicControl = new ZModicControlDialog(this);
    zModicControl->setModel(walletModel);
    zModicControl->exec();
}

void PrivacyDialog::setZModicControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMODICSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMODIC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ModernInvestmentCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMODICpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMODIC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MODIC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMODIC</b>";
    ...
    // use mints from zMODIC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZModicControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZModicControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMODIC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zMODIC"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZMODIC_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMODIC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zMODIC"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zMODIC was spent successfully update the addressbook with the label
    ...
    // Clear zmodic selector in case it was used
    ZModicControlDialog::setSelectedMints.clear();
    ui->labelzMODICSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMODIC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Modic, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMODIC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zmodicTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMODIC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMODIC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMODIC "));
    ui->labelzMODICAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMODICSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzMODIC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zMODIC
        ui->pushButtonMintzMODIC->setEnabled(false);
        ui->pushButtonMintzMODIC->setToolTip(tr("zMODIC is currently disabled due to maintenance."));

        // Spend zMODIC
        ui->pushButtonSpendzMODIC->setEnabled(false);
        ui->pushButtonSpendzMODIC->setToolTip(tr("zMODIC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zMODIC
        ui->pushButtonMintzMODIC->setEnabled(true);
        ui->pushButtonMintzMODIC->setToolTip(tr("PrivacyDialog", "Enter an amount of MODIC to convert to zMODIC", 0));

        // Spend zMODIC
        ui->pushButtonSpendzMODIC->setEnabled(true);
        ui->pushButtonSpendzMODIC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}