...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MODIC)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sMODICPercentage, QString& szMODICPercentage)
{
    int nPrecision = 2;
    ...
    szMODICPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sMODICPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount modicAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = modicAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // MODIC labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, modicAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelMODICPercent->setText(sPercentage);
    ui->labelzMODICPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zMODIC.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in modic.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in modic.conf");
    }
    ...
    bool showMODICAvailable = settingShowAllBalances || modicAvailableBalance != nTotalBalance;
    bool showWatchOnlyMODICAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showMODICPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyMODICPending = watchUnconfBalance != 0;
    bool showMODICLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyMODICLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showMODICAvailable || showWatchOnlyMODICAvailable);
    ui->labelBalanceText->setVisible(showMODICAvailable || showWatchOnlyMODICAvailable);
    ui->labelWatchAvailable->setVisible(showMODICAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showMODICPending || showWatchOnlyMODICPending);
    ui->labelPendingText->setVisible(showMODICPending || showWatchOnlyMODICPending);
    ui->labelWatchPending->setVisible(showMODICPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showMODICLocked || showWatchOnlyMODICLocked);
    ui->labelLockedBalanceText->setVisible(showMODICLocked || showWatchOnlyMODICLocked);
    ui->labelWatchLocked->setVisible(showMODICLocked && showWatchOnly);
    ...
    bool showzMODICAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzMODICUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzMODICImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzMODICAvailable);
    ui->labelzBalanceMatureText->setVisible(showzMODICAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzMODICUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzMODICUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzMODICImmature);
    ui->labelzBalanceImmatureText->setVisible(showzMODICImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelMODICPercent->setVisible(showPercentages);
    ui->labelzMODICPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("MODIC")
    updateDisplayUnit();
}
