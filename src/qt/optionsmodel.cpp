...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MODIC);
    ...
    if (!settings.contains("nAnonymizeModernInvestmentCoinAmount"))
        settings.setValue("nAnonymizeModernInvestmentCoinAmount", 1000);

    nAnonymizeModernInvestmentCoinAmount = settings.value("nAnonymizeModernInvestmentCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeModernInvestmentCoinAmount"))
        SoftSetArg("-anonymizemodicamount", settings.value("nAnonymizeModernInvestmentCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in modic.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeModernInvestmentCoinAmount:
            return QVariant(nAnonymizeModernInvestmentCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeModernInvestmentCoinAmount:
            nAnonymizeModernInvestmentCoinAmount = value.toInt();
            settings.setValue("nAnonymizeModernInvestmentCoinAmount", nAnonymizeModernInvestmentCoinAmount);
            emit anonymizeModernInvestmentCoinAmountChanged(nAnonymizeModernInvestmentCoinAmount);
            break;
        case CoinControlFeatures:
