...
#include "ui_zmodiccontroldialog.h"
...
std::set<std::string> ZModicControlDialog::setSelectedMints;
std::set<CMintMeta> ZModicControlDialog::setMints;

ZModicControlDialog::ZModicControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZModicControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZModicControlDialog::~ZModicControlDialog()
{
    delete ui;
}

void ZModicControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZModicControlDialog::updateList()
{
...
void ZModicControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZModicControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZModic_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZModicControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZModicControlDialog::GetSelectedMints()
{
...
void ZModicControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
