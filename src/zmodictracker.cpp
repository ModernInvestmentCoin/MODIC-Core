...
CzMODICTracker::CzMODICTracker(std::string strWalletFile)
{
...
CzMODICTracker::~CzMODICTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMODICTracker::Init()
{
...
bool CzMODICTracker::Archive(CMintMeta& meta)
{
...
bool CzMODICTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMODICTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMODICTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMODICTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMODICTracker::GetSerialHashes()
{
...
CAmount CzMODICTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMODICTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMODICTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMODICTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMODICTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMODICTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMODICTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMODICTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMODICTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMODICTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMODICTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMODICTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMODICTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMODICTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMODICTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMODICTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMODICTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmodic from DB\n", __func__, listDeterministicDB.size());
    }
